// components/feed/FeedCardSSR.tsx
// Server Component - No "use client" directive
// This component renders feed cards on the server for instant initial display
...
interface FeedCardSSRProps {
  item: FeedItem;
  index?: number;
  isOpen: boolean; // Pre-calculated on server
}

/**
 * Server-Side Rendered Feed Card
 * 
 * Key optimizations:
 * 1. No useRouter - uses native <Link>
 * 2. No useState for visibility - uses CSS animation-delay
 * 3. isOpen calculated on server, passed as prop
 * 4. Image error handling via onError in a minimal client wrapper
 */
export function FeedCardSSR({ item, index = 0, isOpen }: FeedCardSSRProps) {
  ...
  return (
    <div
      className="opacity-0 translate-y-8 animate-fade-in-up h-full"
      style={{
    ...
    >
      <Link href={href} className="block h-full">
        <Card className="group overflow-hidden border border-gray-100 shadow-md hover:shadow-2xl transition-all duration-500 bg-white rounded-[2rem] h-full flex flex-col cursor-pointer">
          {/* Image Section */}
