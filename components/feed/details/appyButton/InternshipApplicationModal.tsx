            ...
            <div className="w-full max-w-2xl origin-center sm:origin-top sm:scale-[0.80] flex justify-center mt-auto sm:my-auto pointer-events-none">
              <DialogPrimitive.Content asChild>
                ...
                <div className="relative bg-[#155DFC] text-white p-6 sm:p-8 shrink-0 overflow-hidden rounded-t-[2.5rem] sm:rounded-t-[2.5rem]">
                  ...
                  {/* Small grab handle for mobile feel */}
                  <div className="absolute top-2 left-1/2 -translate-x-1/2 w-12 h-1.5 bg-white/30 rounded-full sm:hidden" />
                  
                  <div className="relative z-10 flex items-start justify-between mt-2 sm:mt-0">
                    <div className="flex items-center gap-4 sm:gap-5">
                      <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl bg-white/15 backdrop-blur-md flex items-center justify-center border border-white/20 shadow-xl">
                        <Briefcase className="w-7 h-7 sm:w-8 sm:h-8 text-white" />
                      </div>
                      <div className="min-w-0">
                        <h2 className="text-xl sm:text-2xl font-black tracking-tight leading-tight truncate">Internship Portal</h2>
                        <div className="flex items-center gap-2 text-blue-100 text-xs sm:text-sm font-bold mt-1.5 uppercase tracking-widest opacity-90">
                    ...
                    <button 
                      onClick={handleClose} 
                      className="p-2 sm:p-3 hover:bg-white/15 rounded-2xl transition-all duration-300 group"
                    >
                    ...
                    {STEPS.map((s, i) => (
                      <React.Fragment key={s.id}>
                        <div className="flex items-center gap-2">
                          <div className={cn(
                            "w-8 h-8 sm:w-9 sm:h-9 rounded-xl flex items-center justify-center text-xs sm:text-sm font-black transition-all duration-500",
                            ...
                            {step > s.id ? <CheckCircle2 size={16} className="sm:w-[18px] sm:h-[18px]" /> : s.id}
                          </div>
                          <span className={cn(
                            "text-[10px] sm:text-xs font-black uppercase tracking-widest hidden sm:block",
                            step === s.id ? "text-white" : "text-white/40"
                           )}>{s.title}</span>
                        </div>
                        {i < STEPS.length - 1 && (
                          <div className={cn(
                            "flex-1 h-0.5 max-w-[20px] sm:max-w-[40px] rounded-full",
                  ...
                  <AnimatePresence mode="wait">
                    {success ? (
                      <motion.div 
                        ...
                        className="p-8 sm:p-12 flex flex-col items-center text-center pb-12 sm:pb-12"
                      >
                        <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-[2rem] bg-blue-50 flex items-center justify-center mb-6 sm:mb-8 shadow-inner">
                          <Rocket size={40} className="text-[#155DFC] sm:w-12 sm:h-12" />
                        </div>
                        <h3 className="text-3xl sm:text-4xl font-black text-slate-900 mb-3 sm:mb-4 tracking-tight">Application Success!</h3>
                        <p className="text-slate-600 text-base sm:text-lg max-w-sm font-medium leading-relaxed">
                          Your application for <span className="text-[#155DFC] font-bold">{internshipTitle}</span> has been received. 
                          Check your email for further instructions.
                        </p>
                        <Button onClick={handleClose} className="mt-8 sm:mt-10 w-full h-14 rounded-2xl bg-[#155DFC] hover:bg-[#1A3CB9] text-white font-black uppercase tracking-widest text-xs shadow-xl shadow-blue-500/20">
                          Return to Dashboard
                        </Button>
                      </motion.div>
                    ) : (
                      <motion.div 
                        ...
                        exit={{ opacity: 0, x: -10 }}
                        className="p-5 sm:p-8 space-y-5 sm:space-y-8 pb-10 sm:pb-8"
                      >
                        {/* Step Header Block */}
                        <div className="flex items-center gap-4 sm:gap-5 p-5 sm:p-6 bg-blue-50/40 rounded-3xl border border-blue-100/50">
                          <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-[#155DFC] flex items-center justify-center shadow-lg shadow-blue-500/15 shrink-0">
                            {React.createElement(STEPS[step-1].icon, { className: "w-6 h-6 sm:w-7 sm:h-7 text-white" })}
                          </div>
                          <div>
                            <h3 className="text-lg sm:text-xl font-black text-slate-900 tracking-tight">{STEPS[step-1].title}</h3>
                            <p className="text-xs sm:text-sm text-slate-500 font-medium">Please complete the required information below.</p>
                          </div>
                        </div>
                        ...
                        {step === 2 && (
                          <div className="space-y-5 sm:space-y-6">
                            ...
                            <FormField label="Motivation Statement" required error={touched.reason ? errors.reason : undefined} hint="Min 30 characters">
                              <textarea
                                className={cn(inputBaseClass, "h-24 sm:h-28 resize-none", errors.reason && touched.reason && "border-rose-300")}
                            ...
                            <FormField label="Expectations" required error={touched.expectations ? errors.expectations : undefined} hint="Min 20 characters">
                              <textarea
                                className={cn(inputBaseClass, "h-24 sm:h-28 resize-none", errors.expectations && touched.expectations && "border-rose-300")}
                        ...
                        {step === 3 && (
                          <div className="space-y-5 sm:space-y-6">
                            <div className="bg-slate-50 rounded-3xl p-5 sm:p-6 border border-slate-200 relative overflow-hidden">
                              <div className="absolute top-0 right-0 w-24 h-24 sm:w-32 sm:h-32 bg-[#155DFC]/5 rounded-full -mr-12 -mt-12 sm:-mr-16 sm:-mt-16" />
                              <h4 className="text-[10px] sm:text-[11px] font-black text-[#155DFC] uppercase tracking-widest mb-4 sm:mb-6 relative z-10">Application Summary</h4>
                              <div className="grid grid-cols-2 gap-x-6 gap-y-4 sm:gap-x-8 sm:gap-y-6 text-xs sm:text-sm relative z-10 font-bold text-slate-900 leading-tight">
                                <div><span className="text-[9px] sm:text-[10px] uppercase font-bold text-slate-400 block mb-1">Full Name</span>{formData.full_name}</div>
                                <div><span className="text-[9px] sm:text-[10px] uppercase font-bold text-slate-400 block mb-1">Institution</span>{formData.school}</div>
                                <div><span className="text-[9px] sm:text-[10px] uppercase font-bold text-slate-400 block mb-1">Domain</span>{formData.domain}</div>
                                <div><span className="text-[9px] sm:text-[10px] uppercase font-bold text-slate-400 block mb-1">Duration</span>{formData.duration}</div>
                              </div>
                            </div>

                            <div className={cn(
                              "p-5 sm:p-6 rounded-3xl border-2 transition-all duration-300",
                              ...
                              <label className="flex items-start gap-3 sm:gap-4 cursor-pointer">
                                <div className="relative mt-1 shrink-0">
                                  ...
                                  />
                                  <div className={cn(
                                    "w-5 h-5 sm:w-6 sm:h-6 rounded-md sm:rounded-lg border-2 flex items-center justify-center transition-all",
                                    formData.is_paid_acknowledgement ? "bg-[#155DFC] border-[#155DFC]" : "bg-white border-slate-300"
                                  )}>
                                    {formData.is_paid_acknowledgement && <CheckCircle2 size={14} className="text-white sm:w-4 sm:h-4" />}
                                  </div>
                                </div>
                                <div className="space-y-1">
                                  <p className="font-black text-slate-900 tracking-tight text-sm sm:text-base">Terms and Conditions</p>
                            ...
                            <FormField label="Additional Comments (Optional)">
                              <textarea
                                className={cn(inputBaseClass, "h-20 sm:h-24 resize-none")}
                ...
                {/* Footer */}
                {!success && (
                  <div className="shrink-0 p-4 sm:p-6 bg-slate-50 border-t border-slate-100 flex items-center justify-between gap-3 sm:gap-4 pb-8 sm:pb-6">
                    <Button 
                      variant="ghost" 
                      onClick={step === 1 ? handleClose : prevStep}
                      className="px-4 sm:px-6 h-12 rounded-xl font-bold text-slate-500 hover:text-slate-900 transition-colors text-sm"
                    >
                      {step === 1 ? "Cancel" : <><ChevronLeft className="w-4 h-4 mr-1 sm:mr-2" /> <span className="hidden sm:inline">Back</span></>}
                    </Button>
                    ...
                    <Button 
                      ...
                      className={cn(
                        "h-12 sm:h-14 px-6 sm:px-10 rounded-xl sm:rounded-2xl font-black uppercase tracking-widest text-[10px] transition-all duration-300 flex-1 sm:flex-none",
                        isStepValid 
