...
export default function ApplicationModal({
  ...
  if (success) {
    ...
    return createPortal(
      <div className="fixed w-full h-full inset-0 flex items-center justify-center bg-black/60 backdrop-blur-lg animate-in fade-in duration-200" style={{ zIndex: 9999 }}>
        <div className="relative w-full max-w-md mx-4 bg-white dark:bg-gray-950 rounded-2xl shadow-2xl p-8 text-center animate-in zoom-in-95 duration-300">
  ...
  return createPortal(
    <div 
      className="w-full h-screen fixed inset-0 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-200"
      style={{ zIndex: 9999 }}
      onClick={(e) => {
    ...
    >
      <div 
        className="relative w-full sm:max-w-2xl bg-white dark:bg-gray-950 sm:rounded-2xl rounded-t-3xl shadow-2xl max-h-[95vh] sm:max-h-[90vh] overflow-hidden flex flex-col border-t sm:border border-gray-200 dark:border-gray-800 sm:m-4 animate-in slide-in-from-bottom sm:zoom-in-95 duration-300"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-200 dark:border-gray-800 bg-gradient-to-r from-blue-600 to-indigo-600 shrink-0">
          ...
          <Button
            ...
            className="h-9 w-9 rounded-full shrink-0 hover:bg-white/20 text-white"
          >
            <X className="h-5 w-5" />
          </Button>
        </div>
          ...
          <div className="px-4 sm:px-6 py-4 sm:py-6 space-y-5">
            ...
            {currentContent.fields.map((field) => {
                ...
                return (
                  <div key={field.name} className="space-y-2">
                    ...
                    <textarea
                      ...
                      rows={4}
                      className="w-full px-4 py-3 rounded-xl border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 focus:border-blue-500 dark:focus:border-blue-400 focus:ring-4 focus:ring-blue-100 dark:focus:ring-blue-900/30 transition-all duration-200 resize-none text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500"
                    />
                  </div>
                );
              }
                ...
                return (
                  <div key={field.name} className="space-y-2">
                    ...
                    <select
                      value={formData[field.name] || ""}
                      onChange={(e) => handleInputChange(field.name, e.target.value)}
                      className="w-full px-4 py-3 rounded-xl border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 focus:border-blue-500 dark:focus:border-blue-400 focus:ring-4 focus:ring-blue-100 dark:focus:ring-blue-900/30 transition-all duration-200 text-gray-900 dark:text-white cursor-pointer appearance-none bg-[url('data:image/svg+xml;base64,')] bg-no-repeat bg-[center_right_1rem]"
                    >
              ...
              if (field.type === "checkbox") {
                return (
                  <div key={field.name} className="flex items-start gap-3 p-4 border-2 border-gray-200 dark:border-gray-700 rounded-xl bg-gray-50 dark:bg-gray-900/50 hover:bg-gray-100 dark:hover:bg-gray-900 transition-all duration-200">
                    <input
                      type="checkbox"
                      id={field.name}
                      checked={formData[field.name] || false}
                      onChange={(e) => handleInputChange(field.name, e.target.checked)}
                      className="mt-0.5 h-5 w-5 rounded-md border-2 border-gray-300 dark:border-gray-600 text-blue-600 focus:ring-4 focus:ring-blue-100 dark:focus:ring-blue-900/30 transition-all duration-200 cursor-pointer"
                    />
                    <label htmlFor={field.name} className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed cursor-pointer select-none flex-1">
                      {field.label}
                    </label>
                  </div>
                );
                ...
                return (
                  <div key={field.name} className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <label
                        htmlFor={field.name}
                        className="flex items-center justify-center gap-2 w-full px-4 py-3 rounded-xl border-2 border-dashed border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-900/50 hover:bg-gray-100 dark:hover:bg-gray-900 hover:border-blue-400 transition-all duration-200 cursor-pointer"
                      >
              ...
              return (
                <div key={field.name} className="space-y-2">
                  ...
                  <input
                    ...
                    onChange={(e) => handleInputChange(field.name, e.target.value)}
                    placeholder={field.placeholder}
                    className="w-full px-4 py-3 rounded-xl border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 focus:border-blue-500 dark:focus:border-blue-400 focus:ring-4 focus:ring-blue-100 dark:focus:ring-blue-900/30 transition-all duration-200 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500"
                  />
                </div>
              );
            })}
        ...
        {/* Footer */}
        <div className="px-4 sm:px-6 py-3 sm:py-4 border-t border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900/50 shrink-0 space-y-3">
          <Button
            onClick={handleSubmit}
            disabled={isSubmitting || !checkFormValid()}
            className="w-full py-3 rounded-xl font-semibold text-base bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white shadow-lg hover:shadow-xl transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isSubmitting ? (
              <>
