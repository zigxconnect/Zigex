...
export default function MyMonthProject({ user, project, isVisitor = false }: Props) {
  ...
  const demo: Project = {
    ...
    mainImage: project?.mainImage || "/projects.png",
    ...
    <div>
          ...
          <div className="p-5">
            <div className="flex items-start gap-3 mb-4">
              <div className="flex-1">
                <h5 className="text-base font-bold text-gray-900">{p.title}</h5>
                <p className="mt-2 text-sm text-gray-600 leading-relaxed">{p.description}</p>
              </div>

              <div className="flex-shrink-0">
                <Link href={p.githubUrl || '#'} target="_blank" className="inline-flex items-center justify-center w-10 h-10 bg-gray-50 rounded-lg border border-gray-200 hover:scale-105 transition-transform">
            ...
            <div className="grid grid-cols-4 gap-2 mb-4">
              {(p.thumbnails || []).slice(0,4).map((t, i) => (
                <div key={i} className="h-16 w-full rounded-lg overflow-hidden bg-gray-50 border border-gray-100">
                  {t ? (
                    <Image src={t} alt={`thumb-${i}`} width={80} height={64} className="object-cover w-full h-full" />
                  ) : (
            ...
            <div className="flex items-center gap-3 pb-4 border-b border-gray-100">
              <div className="flex -space-x-3">
                {(p.developers || []).slice(0,5).map((d, i) => (
                  <div key={i} className="w-9 h-9 rounded-full ring-2 ring-white overflow-hidden shadow-sm bg-white">
                    {d?.src ? (
            ...
            <button
              ...
              className="mt-4 w-full inline-flex cursor-pointer items-center justify-center gap-2 px-4 py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-all hover:shadow-md font-semibold text-sm"
            >
      ...
      <aside id="my-month-project-panel" className="fixed right-6 top-32 w-72 lg:w-80 bg-white rounded-2xl border-2 border-blue-200 shadow-xl overflow-hidden hidden md:block">
        ...
        <div className="p-4">
          ...
          <button
            onClick={() => setShowContributeModal(true)}
            className="cursor-pointer mt-4 w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-all hover:shadow-md font-medium text-sm"
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <path d="M12 5v14M5 12h14" />
            </svg>
      ...
      {/* Mobile sliding panel - slides in from left (only if NOT visitor) */}
      {!isVisitor && (
        <aside
          className={
        ...
        >
          ...
          <div className="p-4 overflow-y-auto h-full pb-20">
            ...
            <button
              onClick={() => setShowContributeModal(true)}
              className="mt-3 w-full cursor-pointer inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-all hover:shadow-md font-medium text-sm"
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                <path d="M12 5v14M5 12h14" />
              </svg>
