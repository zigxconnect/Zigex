...
const appliedInternships = [
  {
    ...
    coverLetter: "I am passionate about frontend development and excited to contribute to TechBam Solutions. My experience with React and UI design will help your team deliver great products."
  },
  {
    id: 2,
    ...
    coverLetter: "I believe my creativity and communication skills make me a strong fit for Bamenda Digital's marketing team. I look forward to learning and growing with you."
  },
  {
    id: 3,
    ...
    coverLetter: "With my strong background in Python and machine learning, I'm excited to contribute to Analytics Pro's data science initiatives."
  },
  {
    id: 4,
    ...
    coverLetter: "My passion for user-centered design and experience with design thinking methodologies make me an ideal candidate for this position."
  }
];
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-2xl w-full max-w-lg sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="p-6 border-b border-gray-200 flex items-center justify-between">
          <div>
            <h2 className="text-xl sm:text-2xl font-bold text-gray-900">{application.title}</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-3">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="text-lg font-semibold text-green-600">{application.salary}</div>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex items-center gap-2"><User size={16} className="text-gray-400" /> {application.userInfo.name}</div>
                <div className="flex items-center gap-2"><Mail size={16} className="text-gray-400" /> {application.userInfo.email}</div>
                <div className="flex items-center gap-2"><Phone size={16} className="text-gray-400" /> {application.userInfo.phone}</div>
                <div className="flex items-center gap-2"><FileText size={16} className="text-gray-400" /> 
                  <span className="text-blue-600 hover:underline cursor-pointer">{application.userInfo.resume}</span>
                </div>
              </div>
            </div>
          </div>
...
const EditModal = ({ application, isOpen, onClose, onSave }) => {
  ...
  useEffect(() => { if (application) setFormData(application); }, [application]);
  if (!isOpen || !application) return null;
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-2xl w-full max-w-lg sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="p-6 border-b border-gray-200 flex items-center justify-between">
          <h2 className="text-xl sm:text-2xl font-bold text-gray-900">Edit Application</h2>
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-4">
            <button onClick={handleSave} className="flex items-center justify-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
...
export default function AppliedInternshipsPage() {
  ...
  const handleView = (application) => setViewModal({ isOpen: true, application });
  const handleEdit = (application) => setEditModal({ isOpen: true, application });
  const handleDelete = (id) => setApplications(applications.filter(app => app.id !== id));
  const handleSave = (updated) => setApplications(applications.map(app => app.id === updated.id ? updated : app));
              ...
              <div className="p-6">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    ...
                    <p className="text-gray-600 flex items-center gap-2 mt-1"><Building size={14} /> {application.company}</p>
                  </div>
                  <StatusBadge status={application.status} />
                </div>

                <div className="space-y-2 text-sm text-gray-600 mb-4">
                  <div className="flex items-center gap-2"><Calendar size={14} /> Applied: {new Date(application.appliedDate).toLocaleDateString()}</div>
                  <div className="flex items-center gap-2"><MapPin size={14} /> {application.location}</div>
                  <div className="flex items-center gap-2"><Clock size={14} /> {application.duration}</div>
                  <div className="font-medium text-green-600">{application.salary}</div>
                </div>

                <div className="flex flex-wrap gap-2">
                  <button onClick={() => handleView(application)} className="flex-1 flex items-center justify-center gap-2 px-3 py-2 text-sm bg-gray-50 text-gray-700 rounded-lg hover:bg-gray-100 transition-colors"><Eye size={14} /> View</button>
                  <button onClick={() => handleEdit(application)} className="flex-1 flex items-center justify-center gap-2 px-3 py-2 text-sm bg-blue-50 text-blue-700 rounded-lg hover:bg-blue-100 transition-colors"><Edit3 size={14} /> Edit</button>
                  <button onClick={() => handleDelete(application.id)} className="flex-1 flex items-center justify-center gap-2 px-3 py-2 text-sm bg-red-50 text-red-700 rounded-lg hover:bg-red-100 transition-colors"><Trash2 size={14} /> Delete</button>
                </div>
              </div>
            </div>
          ))}
